...
使用 React Three Fiber 作为 React 与 Three.js 的集成层，使用 Drei 提供相机控制、标签和加载辅助。Kitchen 使用 GLB/GLTF 资产，按需加载，不影响其他页面启动。

场景不能是不可拆分的单一网格。每个可学习对象都必须拥有稳定的 `object_key`，并与 GLB 节点名及数据库记录一致，例如 `sink`、`faucet`、`refrigerator`、`stove` 和 `cutting_board`。
...
- GLB 加载失败或 WebGL 不可用：切换到按对象分组的列表模式，保留学习、TTS、测试和收藏能力。
