...
import { SparkIcon, PlayIcon, UpvoteIcon, ReplyIcon, ExternalIcon, TranslateIcon } from "@/components/ui/icons";
...
export function EntryCard({ entry, feedTitle, hero = false, learning, onOpen, onLearn, onPlay, onTranslate, translating = false, trackRead = true, coverColor, coverLetter }: Props) {
  const t = useT();
...
export function EntryListRow({ entry, feedTitle, learning, onOpen, onLearn, onPlay, onTranslate, translating = false, trackRead = true }: Props) {
  const t = useT();
  const unread = trackRead && !entry.is_read;
  ...
  return (
    ...
    >
      ...
      <h3 className={`min-w-0 flex-1 truncate text-[13.5px] ${unread || !trackRead ? "font-medium text-foreground" : "text-muted-foreground"}`}>
        {entry.title}
      </h3>
      {(entry.points != null || entry.commentCount != null) && (
