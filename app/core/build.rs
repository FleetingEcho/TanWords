...
fn main() {
    ...
    // sherpa-rs (TTS) dylibs are bundled into Resources/sherpa-libs/ (or, on
    // Linux, usr/lib/<ProductName>/sherpa-libs/) by electron-builder's
    // `extraResources` (see sherpa-libs/ above) — the packaging step
    // preserves the source glob's relative directory rather than flattening
    // it. Add that directory to the binary's rpath so dyld/the dynamic
...
fn generate_dispatch_table() {
    ...
    let header = "// GENERATED by build.rs — do not edit by hand.\n\
                  ...
                  // the two explicit imports puts those bare names in scope for this module\n\
    ...
    println!(
        "cargo:warning=dispatch: skipped {} (reimplemented in Electron main): {}",
        skipped.len(),
        skipped.join(", ")
    );
