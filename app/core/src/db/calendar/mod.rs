...
// ── calendars (colour categories): CRUD submodule ───────────────────────────
//
// The four `calendar_calendars` CRUD commands live in the `calendars`
// submodule (split out so this file stays under 600 lines). `pub use`
// re-exports them here, so the public `calendar::…` / `db::…` paths — and the
// generated dispatch call `db::calendar::db_list_calendar_calendars` — are
// unchanged. The shared `sanitize_id` / `uuid_id` helpers and the
// `CalendarCategory` wire type stay in this file; the submodule names them via
// `use super::{…}` (a child module can reach its parent's private items).
mod calendars;
pub use calendars::*;
