...
### Documents 与 AI Chat —— 随时一键呼出

一个悬浮工具球会打开一个可拖拽的窗口，里面有个人笔记编辑器（BlockNote、全文
搜索、标签）和多会话 AI 对话——在任何页面都能直接呼出，不必离开当前正在看的
内容。

![悬浮工具窗口 — Documents 标签页](static/images/Documents.png)

![悬浮工具窗口 — AI Chat 标签页](static/images/AI%20Chat.png)
...
| Feeds（订阅） | 同时订阅文章类 RSS 源和播客；浏览 HackerNews，通过应用内阅读器或粘贴方式把文章导入 Reading；应用内阅读器同样支持"朗读全文"；播客集在底部常驻播放条中播放。 |
| Scene Lab（场景学习） | 生成适配 CEFR 的 Kitchen 课程，在 20 个低多边形 3D 物体中探索词汇、进行空间回忆和任务链练习，并把薄弱词加入词库/FSRS。 |
...
| Documents（文档） | 个人笔记编辑器（BlockNote）、全文搜索（SQLite FTS5）、标签、置顶 —— 也可通过悬浮工具窗口在任意页面呼出。 |
| AI Chat（AI 对话） | 支持工具调用、可直接读写应用数据的多会话对话 —— 也可通过悬浮工具窗口在任意页面呼出。 |
