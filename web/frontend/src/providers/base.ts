...
/** Condenses an upstream provider error body into something a toast/bubble
 *  can show. Gateways can answer with whole HTML pages (502s) or nested JSON
 *  envelopes ({"error":{"message":...}}); the first ~600 chars of the plain
 *  text are kept so matcher heuristics like isContextOverflowError() still see
 *  the upstream's original phrasing. Never includes keys — on web the client
 *  carries none, and the proxy strips its own upstream credential. */
export function condenseUpstreamError(body: string): string {
  let text = body.trim();
  try {
