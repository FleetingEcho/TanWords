import type { Dict } from "../types";

export const browser: Dict = {
    "browser.addressPlaceholder": "Search or enter a URL",
    "browser.back": "Back",
    "browser.forward": "Forward",
    "browser.reload": "Reload",
    "browser.clearData": "Clear data",
    "browser.home": "Home",
    "browser.askSelection": "Ask about text",
    "browser.askPaneTitle": "Ask about text",
    "browser.askPlaceholder": "Paste text from the page here",
    "browser.askNew": "New text",
    "browser.askResize": "Resize panel",
    "browser.newTab": "New tab",
    "browser.closeTab": "Close tab",
    "browser.openInDefaultBrowser": "Open in default browser",
    "browser.clearingData": "Clearing…",
    "browser.clearDataConfirmTitle": "Clear browsing data?",
    "browser.clearDataConfirmMessage": "This signs you out of any sites you're logged into here and clears cookies, local storage, and cache for the embedded browser. This can't be undone.",
    "browser.clearDataDone": "Browsing data cleared",
    "browser.emptyTitle": "Browse without leaving TanWords",
    "browser.emptyBody": "Paste any link above, or start from one of these.",
};
