import { assetUrlById, invoke } from "@/api/client";
...
/** BlockNote `resolveFileUrl`: stored `tanwords-asset://<id>` -> a real HTTP
 *  URL on the same origin (`/api/assets/:id`, bearer token embedded by
 *  assetUrlById). Web never hands out path-based URLs — the server gates the
 *  route on document privacy, so assets of locked documents stay sealed.
 *  Straight pass-through: no base64 round trip, and the browser gets proper
 *  caching/streaming for big media for free. */
export function resolveDocumentAssetUrl(url: string): Promise<string> {
  if (!url.startsWith(DOCUMENT_ASSET_SCHEME)) return Promise.resolve(url);
  return Promise.resolve(assetUrlById(url.slice(DOCUMENT_ASSET_SCHEME.length)));
}
