...
import { downloadText, pickFiles } from "@/api/platform";
...
export function useDocImportExport(params: {
  ...
  const exportDocuments = useCallback(async (ids: number[]) => {
    try {
      ...
      // Browsers can't target a chosen folder — each exported markdown file
      // becomes a plain download instead.
      const files = [];
      for (const id of ids) {
      ...
      for (const file of files) downloadText(file.name, (file as { markdown?: string }).markdown ?? "");
