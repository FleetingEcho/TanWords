...
import { assetUrlById, authFetch } from "@/api/client";
...
export function useDocumentAssetManager() {
  ...
  /** Browsers have no save dialog: asset export goes through the web server's
   *  /api/assets/:id endpoint and lands in the user's download folder. */
  const downloadAssets = async (list: DocumentAssetSummary[], nameFor: (a: DocumentAssetSummary) => string) => {
    for (const asset of list) {
      const response = await authFetch(assetUrlById(asset.id));
  ...
  return {
    ...
    exportOne, confirmDelete, cleanOrphans, exportSelectedToFolder, deleteSelected,
  };
}
