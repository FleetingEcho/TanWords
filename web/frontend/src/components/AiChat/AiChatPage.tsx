...
import { ArrowDown, Bot, ChevronDown, Eraser, FilePlus2, PanelLeft, PlugZap, Unplug } from "lucide-react";
...
export function AiChatPage({ initialSessionId, onActiveIdChange }: { initialSessionId?: string; onActiveIdChange?: (id: string | null) => void } = {}) {
  ...
  return (
    // Explicit pixel height below lg: visualViewport shrinks under the mobile
    // keyboard while 100%/dvh keeps measuring the unobscured screen — without
    // this the composer docks behind the keyboard (see useMobileViewportHeight).
    <div
      style={viewportHeight != null ? { height: viewportHeight } : undefined}
      className={`flex h-full overflow-hidden overscroll-none ${hasCustomAppBackground ? "" : "bg-background"}`}
    >
      {/* Desktop column */}
      ...
      <main className="min-w-0 flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-2 px-3 lg:px-5 h-16 border-b border-border/60 bg-background/65 backdrop-blur-xl shrink-0">
          <Button
            ...
            className="h-9 w-9 rounded-xl p-0 text-muted-foreground hover:bg-muted hover:text-foreground shrink-0 lg:hidden"
          >
          ...
          {s.providers.length > 0 ? (
            <Select value={s.selectedProviderId} onValueChange={s.setSelectedProviderId}>
              <SelectTrigger
                ...
                className="h-9 w-auto max-w-[150px] lg:max-w-[240px] gap-2 rounded-xl border-border/70 bg-card/70 px-2.5 text-xs shadow-none focus:ring-1 focus:ring-primary/20 shrink-0"
              >
