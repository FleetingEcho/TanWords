...
export function ProviderSection() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // (Web: keys aren't readable client-side; `hasKey` is the signal here.)
      const currentDefault = useSettingsStore.getState().defaultAiProvider;
      if (!loadedConfigs[currentDefault]?.hasKey) {
        const firstWithKey = Object.values(loadedConfigs).find((c) => c.hasKey)?.id;
  ...
  const testConnection = async (providerId: string, apiBase: string, apiKey: string, modelId?: string) => {
    ...
    // Web: go through the same-origin AI proxy, which resolves base URL +
    // credentials from the saved DB row — so this only works once the
    // provider has been saved (apiBase/apiKey args still flow in for the
    // desktop-shaped call sites above; only the id is used for routing).
    // TODO(web-wave): pre-save validation (testing form values before the row
    // exists) needs a server "validate these values" endpoint.
    void apiBase;
  ...
  const fetchModels = async (providerId: string, apiBase: string, apiKey: string, selectFirst: (model: string) => void, currentModel: string) => {
    if (!apiBase.trim() && providerId !== "openai" && providerId !== "claude") return;
    setFetchingModels(true);
    try {
      // Web: same proxy routing as testConnection (saved rows only).
      const base = `/api/ai-proxy/${encodeURIComponent(providerId)}`;
