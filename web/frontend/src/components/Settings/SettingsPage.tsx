...
// Web scope: no TTS (on-device voices) and no MCP section (server config only).
const SECTIONS = ["general", "providers", "learning", "documents", "data"] as const;
...
export function SettingsPage() {
  ...
  const sectionRefs = useRef<Record<SectionId, HTMLElement | null>>({
    general: null, providers: null, learning: null, documents: null, data: null,
  });
  ...
  return (
    <div className="flex h-full flex-col animate-fade-in">
      ...
      <div className="flex shrink-0 gap-1 overflow-x-auto border-b border-border px-4 sm:px-8 py-2">
        {SECTIONS.map((id) => (
          <button
            key={id}
            onClick={() => jumpTo(id)}
      ...
      <div ref={scrollRef} className="relative flex-1 overflow-y-auto">
        ...
        <div className="max-w-full px-4 sm:px-8 py-5 sm:py-6 space-y-8 sm:space-y-10" data-no-selection>
