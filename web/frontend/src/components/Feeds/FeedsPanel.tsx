...
import { openExternal as openShell } from "@/api/platform";
...
/** One-time cleanup: the hnrss.org RSS subscription is superseded on desktop by
 *  the native Hacker News section, and on web by nothing at all (HN is post-v1).
 *  Drop it from legacy installs so it doesn't show up as an article tab. */
const HN_RSS_URL = "https://hnrss.org/frontpage?points=100";
const HN_NATIVE_MIGRATED_FLAG = "tanwords_hn_native_migrated";

export type FeedPanelKind = "article" | "podcast";

/** Default-feed seeding is app-global (the seed set covers both articles and
 *  podcasts), not per-tab — but it must not run at app boot, only once the
 *  user has actually landed on a feeds/podcasts tab. Both FeedsPanel instances
 *  share this latch so the seed runs exactly once, on first visit. */
let seedPromise: Promise<RssFeed[]> | null = null;
function seedOnce(db: ReturnType<typeof useDB>): Promise<RssFeed[]> {
  if (!seedPromise) seedPromise = db.getRssFeeds().then((list) => seedDefaults(db, list));
  return seedPromise;
}

/** Which panel a browsed entry belongs to: explicit when opened from a panel,
 *  inferred from audio for rows opened from bookmarks/recents/dashboard. */
function browseKind(browse: { kind?: "article" | "podcast"; audioUrl?: string | null } | null): FeedPanelKind {
  if (!browse) return "article";
  return browse.kind ?? (browse.audioUrl ? "podcast" : "article");
}

/** One feeds surface for the Reading page: subscribed content of a single kind
 *  (articles or podcasts) with the full pinned-tabs / More-library / bookmarks /
 *  recents chrome. The two instances are siblings under ReadingPage's segmented
 *  control and stay mounted once visited, so an open entry survives tab hops. */
export function FeedsPanel({ kind }: { kind: FeedPanelKind }) {
  const t = useT();
  const db = useDB();
  ...
  const [selected, setSelected] = useState<RssTabSelection>("all");
  ...
  /** This panel's slice of the feed list — articles and podcasts each have
   *  their own surface in the merged Reading page. */
  const kindFeeds = useMemo(
    () => feeds.filter((f) => (kind === "podcast" ? f.category === "podcast" : f.category !== "podcast")),
    [feeds, kind]
  );
  const kindIdsSet = useMemo(() => new Set(kindFeeds.map((f) => f.id)), [kindFeeds]);
  const kindIdsRef = useRef(kindIdsSet);
  useEffect(() => { kindIdsRef.current = kindIdsSet; }, [kindIdsSet]);
  const kindUnread = useMemo(
    () => new Map([...unreadByFeed].filter(([id]) => kindIdsSet.has(id))),
    [unreadByFeed, kindIdsSet]
  );

  const refreshEntries = useCallback(async (sel: RssTabSelection) => {
    // Web has no Hacker News tab; clamp the desktop-only value defensively
    // (a settings profile synced from desktop can still carry it).
    const rows = await db.getRssEntries(sel === "all" || sel === "hackernews" ? null : sel);
    ...
    if (selectedRef.current === sel) {
      const ids = kindIdsRef.current;
      setEntries(rows.filter((r) => ids.has(r.feed_id)));
    }
  ...
  // background sync bringing in new entries.
  useEffect(() => {
  ...
  // idle window so navigating to the tab remains responsive. Seeding happens
  // here (lazily, once across both panels) — not at app start.
  useEffect(() => {
    let syncTimer: number | undefined;
    (async () => {
      try {
        let list = await seedOnce(db);
        ...
        setFeeds(list);
        // Respect the user's configured default tab only when it names a feed
        // of *this* kind (desktop installs may point at any kind, or HN).
        const requestedTab = useSettingsStore.getState().defaultRssTab;
        const validTab: RssTabSelection =
          typeof requestedTab === "number" && list.some((f) => f.id === requestedTab && (kind === "podcast" ? f.category === "podcast" : f.category !== "podcast"))
            ? requestedTab
            : "all";
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- one-shot mount: seed + first paint only.
  }, []);

  // Category retoggles / deletions can strand the selection on a feed this
  // panel no longer owns — fall back to the kind's aggregate view.
  useEffect(() => {
    if (selected !== "all" && selected !== "hackernews" && !kindIdsSet.has(selected)) {
      selectedRef.current = "all";
      setSelected("all");
      void refreshEntries("all");
    }
  }, [kindIdsSet, selected, refreshEntries]);
  ...
  const openBookmark = (bookmark: FeedBookmark) => {
    const target: BrowseTarget = {
      ...
      kind: bookmark.audio_url ? "podcast" : "article",
    };
    setBrowse(target);
    addRecentlyRead(target);
    setRecentlyRead(getRecentlyRead());
  ...
  const openRecent = (item: RecentlyReadItem) =>
    setBrowse({ ...item, kind: item.audioUrl ? "podcast" : "article" });
   ...
   *  on the tab, same headless job as ArticleReader's "Learn" button (useLearnArticle),
  ...
  // A browse belongs to exactly one panel; the sibling stays on its grid.
  const panelBrowse = browse && browseKind(browse) === kind ? browse : null;

  return (
    <div className="flex h-full flex-col animate-fade-in">
      <FeedTabs
        feeds={kindFeeds}
        unreadByFeed={kindUnread}
