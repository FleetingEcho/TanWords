...
/** A blockquote line counts as an example sentence if it looks
 * predominantly English (more Latin letters than CJK characters). This lets
 * a translation line in the same blockquote be skipped by anything that
 * filters the example rows (e.g. the old speak buttons) without needing the
 ...
 *
 * The per-example gutter sits beside rather than trailing the sentence.
 * Trailing reads better when it fits, but an example that happens to fill its
 * last line pushes the affordance onto a line of its own — an orphaned icon
 * under the text. A gutter puts it in the same place on every card instead. */
 ...
 * providers/base.ts buildEnrichSystemPrompt) so they render as example cards;
 * the layout gutter stays for future affordances (the old speak buttons were
 * dropped with desktop TTS).
