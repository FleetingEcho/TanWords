...
import { openExternal as openUrl } from "@/api/platform";
import {
  BrainCircuit, Check, ChevronsLeft, ChevronsRight, ClipboardPaste, Cloud, CloudOff, Database,
  FilePlus2, Languages, MessageSquarePlus, Monitor, Moon, Palette, Quote, Search, Settings, Sun,
...
const PAGE_IDS: NavPage[] = ["dashboard", "vocabulary", "reading", "chat", "documents", "settings"];
...
export function CommandBar({ activePage }: { activePage: NavPage }) {
  ...
  // The MCP status button/polling of the desktop build is gone on web — MCP
  // is server config only (no Settings section, and its invoke would 404 on
  // the web server's reduced command set).
  ...
  return (
    <>
      ...
      <header className="app-drag-region relative z-30 flex h-12 shrink-0 select-none items-center gap-1.5 border-b border-border/80 bg-background/90 px-3 backdrop-blur-xl">
        ...
        {/* TTS is out of web scope (no on-device voices) — the desktop's
          * TtsControl here is removed. */}
