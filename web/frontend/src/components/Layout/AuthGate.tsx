...
import { login, register as registerAccount, resetPassword } from "@/api/auth";
...
function Field(props: {
  ...
  return (
    <label className="flex flex-col gap-1.5 text-sm">
      ...
      {/* text-[16px] is deliberate: iOS zooms focused inputs below 16px. */}
      <input
        type={props.type}
...
/** Full-screen gate shown until the device holds a valid session token.
 *
 *  Three in-page screens switched by local state (no router): login, register,
 *  and password reset. Register and reset require the server owner's invite
 *  key; register lands straight in the app (the response carries a token),
 *  reset returns to login with a success notice. On a successful
 *  login/register, setToken fires 'tanwords:authorized' and App unmounts this
 *  gate entirely. */
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      if (mode === "login") {
        ...
        // setToken fired 'tanwords:authorized' — App unmounts this gate.
      } else if (mode === "register") {
        await registerAccount(email.trim(), password, inviteKey.trim());
        // Same: token landed, gate unmounts.
      } else {
      ...
      // Server errors arrive as bare strings (see api/auth.ts) — show as-is.
