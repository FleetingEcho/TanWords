...
export function VocabularyPage({ initialWordId, initialSentenceId }: { initialWordId?: number; initialSentenceId?: number }) {
  ...
  useEffect(() => {
    if (view !== "review") void db.getReviewCount().then(setDueCount).catch(() => {});
  }, [db, view]);
    ...
    <div className="flex h-full flex-col">
      <div className="flex shrink-0 items-center justify-between gap-3 border-b border-border px-4 py-2.5 bg-transparent">
        <div className="flex items-center gap-1">
          {(["words", "patterns", "review"] as const).map((tab) => (
            <button
              key={tab}
              onClick={() => setView(tab)}
            ...
            >
              {t(tab === "words" ? "vocab.tabWords" : tab === "patterns" ? "vocab.tabPatterns" : "vocab.tabReview")}
              {tab === "review" && dueCount > 0 && (
                <span className="ml-1.5 rounded-md bg-primary/15 px-1.5 py-0.5 text-[10px] font-bold text-primary">{dueCount}</span>
              )}
            </button>
          ))}
        </div>
      ...
      {view === "patterns" ? (
        ...
        <ReviewPanel
          onExit={() => setView("words")}
          onFinished={() => void db.getReviewCount().then(setDueCount).catch(() => {})}
        />
      ) : (<>
      ...
      {narrow && (selected || lookup) && (
        <div className="fixed inset-0 z-50 flex flex-col bg-background lg:hidden">
          ...
          <div className="min-h-0 flex-1">{wordDetail}</div>
        </div>
      )}
      </>)}
