...
export function useArticleReaderState(params: {
  ...
  const playerActive = podcastActive;
  ...
  const handleListen = () => {
    ...
    // Reading article text aloud (TTS) is desktop-only; the web reader's
    // listen button exists only when a podcast episode is attached.
  };
  ...
  return {
    ...
    podcastActive, podcastStatus, playerActive,
    ...
    handleCopyMarkdown, handlePasteSubmit, handleHnCommentsLoaded,
  };
}
